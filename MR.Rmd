...
```{r, echo=F}
team_plot_data = readxl::read_xlsx("C:/Users/harri/OneDrive/Desktop/LTB/CFB Analysis/natty.xlsx")

team_plot_data <- CFB_MODEL(ht=ht,at=at,input_week=1,input_season=2022,conferences = c(),previous_season=1,remove_fcs = TRUE)
  ...
  gt_fmt_cfb_logo(columns = c('Team')) %>% 
   ...
   def_rushing_plays_explosiveness = md("**Rush EXP**"),
    ...
    data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_success_rate),max(TEAMS$def_success_rate)
            )
        )
    ) %>% 
        data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_explosiveness),max(TEAMS$def_explosiveness)
            )
        )
    )  %>% 
        data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_power_success),max(TEAMS$def_power_success)
            )
        )
    ) %>% 
        data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_passing_plays_success_rate),max(TEAMS$def_passing_plays_success_rate)
            )
        )
    ) %>% 
        data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_passing_plays_explosiveness),max(TEAMS$def_passing_plays_explosiveness)
            )
        )
    ) %>% 
    data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_rushing_plays_success_rate),max(TEAMS$def_rushing_plays_success_rate)
            )
        )
    ) %>% 
    data_color(
        ...
        colors = scales::col_numeric(
            palette = c('green','white','red'),
            domain = c(min(TEAMS$def_rushing_plays_explosiveness),max(TEAMS$def_rushing_plays_explosiveness)
            )
        )
    ) %>% 
    tab_header(
