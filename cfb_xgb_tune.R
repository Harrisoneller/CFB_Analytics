...
for(conf in 1:length(conferences)){
 ...
 # if(is.null(remove) == F){at = at[-remove]}
  ...
  for (j in 1:length(ht)){
    ...
    # home_stats1 = cfbd_game_team_stats(2021, team = ht[j])
    # away_stats1 = cfbd_game_team_stats(2021, team = at[j])
    #
    # home_advanced1 = cfbd_stats_game_advanced(2021, team = ht[j])
    # away_advanced1 = cfbd_stats_game_advanced(2021, team = at[j])
    #
    # home_ppa1 = cfbd_metrics_ppa_games(2021, team = ht[j])
    # away_ppa1 = cfbd_metrics_ppa_games(2021, team = at[j])
    #
    #
    #
    # home_elo1 = cfbd_ratings_elo(year = 2021,team = ht[j])
    # away_elo1 = cfbd_ratings_elo(year = 2021,team = at[j])
    #
    #
