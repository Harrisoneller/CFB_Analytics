...
CFB_PROJECTIONS <- function(ht=c(),at=c(),input_week,input_season,conferences = c(),include_vegas = 1,previous_season=0,remove_fcs = TRUE,pbp_full){
  ...
  for (j in 1:length(ht)){
    ...
    ht_pbp_off<-pbp_full %>% 
      filter(pos_team == ht[j]) 
    ht_pbp_def<-pbp_full %>% 
      filter(def_pos_team == ht[j]) 
    
    ht_EPA <- ht_pbp_off %>% 
      group_by(game_id) %>% 
      summarize(avg_epa = mean(EPA,na.rm = T))
    
    ht_EPA_allowed <- ht_pbp_def %>% 
      group_by(game_id) %>% 
      summarize(avg_epa_allowed = mean(EPA,na.rm = T))
    
    at_pbp_off<-pbp_full %>% 
      filter(pos_team == at[j]) 
    at_pbp_def<-pbp_full %>% 
      filter(def_pos_team == at[j]) 
    
    at_EPA <- at_pbp_off %>% 
      group_by(game_id) %>% 
      summarize(avg_epa = mean(EPA,na.rm = T))
    
    at_EPA_allowed <- at_pbp_def %>% 
      group_by(game_id) %>% 
      summarize(avg_epa_allowed = mean(EPA,na.rm = T))
    
    ht_EPA_full <- inner_join(ht_EPA,ht_EPA_allowed, by = 'game_id')
    at_EPA_full <- inner_join(at_EPA,at_EPA_allowed, by = 'game_id')
    ...
    home <- inner_join(home,ht_EPA_full, by = 'game_id')
    away <- inner_join(away,at_EPA_full, by = 'game_id')
    ...
    if (include_vegas ==1){
      ...
      new_data_home = data.frame(#elo = recent_home$elo[nrow(recent_home)], 
        ...
        avg_epa = (median(as.numeric(recent_home$avg_epa)) + median(as.numeric(recent_away$avg_epa_allowed)))/2,
        avg_epa_allowed = (median(as.numeric(recent_home$avg_epa_allowed)) + median(as.numeric(recent_away$avg_epa)))/2 )
      # 
      ...
      new_data_away = data.frame(#elo = recent_away$elo[nrow(recent_away)], 
        ...
        avg_epa = (median(as.numeric(recent_away$avg_epa)) + median(as.numeric(recent_home$avg_epa_allowed)))/2,
        avg_epa_allowed = (median(as.numeric(recent_away$avg_epa_allowed)) + median(as.numeric(recent_home$avg_epa)))/2 )
...
for (conf in c("SEC","ACC","B1G","B12",'PAC')){
  y <- CFB_PROJECTIONS(ht=c(),at=c(),input_week=9,input_season=2023,include_vegas=1,conferences = c(conf),previous_season=0,remove_fcs = TRUE,pbp_full = pbp_full)
  games<-rbind(games,y,fill=TRUE)
}
...
# y <- CFB_PROJECTIONS(ht=c("Texas"),at=c("BYU"),input_week=9,input_season=2023,include_vegas=1,conferences = c(),previous_season=0,remove_fcs = TRUE)
# 
# df$value_side[5]<-'Texas'
# p5<-df
